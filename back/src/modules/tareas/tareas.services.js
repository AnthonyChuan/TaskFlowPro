...
import { actualizarTareaDone, actualizarTareaInProgress, actualizarTareaInReview, contarTareasPorId, crearTarea, selectTareasPorId, validarDuenoTarea, validarProjectId, validarTarea, validarUserId } from "./tareas.repository.js";
...
export async function selectTareasService(id_usuario,page,limit) {
    const client= await pool.connect()
    try {
        const offset = (page - 1) * limit
        const tareas = await selectTareasPorId(client, { id_usuario,limit, offset })
        const total  = await contarTareasPorId(client,id_usuario)
        return {
            tareas,
            meta: {
                page,
...
export async function actualizarTareaEnCursoDeveloperService(id_tarea,id) {
    ...
    try {
        ...
        if (tarea.assigned_to!==id) {
            const error=new Error("No puedes cambiar el estado de la tarea de otro")
            error.status=401
            throw error
        }
